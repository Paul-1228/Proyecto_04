...
# Para la primera parte de la modulación 16QAM lo que se hace es modificar las funciones modulador y demodulador para adaptarlas a los requerimientos. 
# 
# Para ello se aplica el mismo procedimiento de la modulación BPSK pero ahora aplicado a dos portadoras de cuatro bits.
...
def modulador(bits, fc, mpp): 
    ...
    #Creación de los portadores
    portador_1 = np.cos(2*np.pi*fc*t_periodo)
    portador_2 = np.sin(2*np.pi*fc*t_periodo)

    # Tiempo total de la simulacion
    t_simulacion = np.linspace(0, (len(bits))*Tc, (len(bits))*mpp)

    #Asignación de un vector de cero para el soporte de la gráfica 
    senal_Tx = np.zeros(t_simulacion.shape)

    #Se hace un for para la comparación de paquetes de dos bits 
    for i in range(0, len(bits), 4):
        senal_Tx[iterador*mpp : (iterador+1)*mpp] =             (-1)**(1+bits[i]) * 3**(1-bits[i+1]) * portador_1             + (-1)**(bits[i+2]) * 3**(1-bits[i+3]) * portador_2
        iterador += 1

    #Se aplica la fórmula
    P_signal_Tx = 1 / ((len(bits))*Tc) * np.trapz(pow(senal_Tx, 2), t_simulacion)

    return senal_Tx, P_signal_Tx, portador_1, portador_2


# In[292]:


def demodulador(signal_Rx, portador_1, portador_2, mpp):
    #Muestras totales
    M = len(signal_Rx) 

    #Número total de la transmisión
    N = int(M / mpp)  

    bits_Rx = np.zeros(N)
    signal_demodulada = np.zeros(signal_Rx.shape)
    iterador = 0
    ...
    for i in range(N):
        if iterador+4 > N:  
            ...
            mul_1 = signal_Rx[i*mpp: (i+1)*mpp] * portador_1
            mul_2 = signal_Rx[i*mpp: (i+1)*mpp] * portador_2
            signal_demodulada[i*mpp: (i+1)*mpp] = mul_1 + mul_2
            
            # Detección de bits
            
            if np.sum(mul_1) >= 0: bits_Rx[iterador] = 1  # bit 1 
            if np.max(np.abs(mul_1)) < 2.5: bits_Rx[iterador+1] = 1  # bit 2
            if np.sum(mul_2) < 0: bits_Rx[iterador+2] = 1  # bit 3
            if np.max(np.abs(mul_2)) < 2.5: bits_Rx[iterador+3] = 1   # bit 4

            iterador += 4
    return bits_Rx.astype(int), signal_demodulada
...
# In[293]:
    ...
    #Modulacion 16-QAM
    senal_Tx, Pm, portador_1, portador_2 = modulador(bits_Tx, fc, mpp)
    print(f"Potencia de emisión: {Pm}")
    signal_Rx = canal_ruidoso(senal_Tx, Pm, SNR)
    bits_Rx, signal_demodulada = demodulador(signal_Rx, portador_1, portador_2, mpp)
    imagen_Rx = bits_a_rgb(bits_Rx, dimensiones)
    Fig = plt.figure(figsize=(10, 6))

    #Tiempo de simulación
    print('Tiempo de la simulación: ', time.time() - inicio)
    ...
    #Se plotea la imagen de entrada
    ax = Fig.add_subplot(1, 2, 1)
    imgplot = plt.imshow(imagen_Tx)
    ax.set_title('Transmitido')
    
    #Se plotea la imagen se salida
    ax = Fig.add_subplot(1, 2, 2)
    imgplot = plt.imshow(imagen_Rx)
    ax.set_title('Recuperado')
    Fig.tight_layout()
    ...
    ax2.plot(senal_Tx[0:600], color='g', lw=2)
    ax2.set_ylabel('$s(t)$')
    ax3.plot(signal_Rx[0:600], color='b', lw=2)
    ax3.set_ylabel('$s(t) + n(t)$')
    ax4.plot(signal_demodulada[0:600], color='m', lw=2)
    ax4.set_ylabel('$b^{\'}(t)$')
    ax4.set_xlabel('$t$ / milisegundos')
    fig.tight_layout()
...
#  Proceso aleatorio X(t) con N operaciones
...
# Grafica el resultado teórico del valor esperado
E = np.mean(senal_Tx)*t
Teo, = plt.plot(t, E, '-.', lw=3, color = 'r', label='Valor teorico')

# Mostrar las realizaciones, su promedio calculado y teórico
...
# Valor teórico de correlación donde la magnitud es (varianza - media^2), con p = 0.5 ya que es una variable de Bernoulli
Rxx = (0.5 - np.power(0.25,2)) * np.cos(np.pi*taus)

# Gráficas de correlación para cada operación
...
# Gráfica
plt.plot(f, 2.0/Nm * np.power(np.abs(signal_f[0:Nm//2]), 2))
plt.xlim(0, 20000)
plt.grid()
plt.show()
